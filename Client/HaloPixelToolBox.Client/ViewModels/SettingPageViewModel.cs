...
{
    [ObservableProperty]
    public partial int CloseButtonActionIndex { get; set; } = SystemProfile.MinimizeWhenClose ? 0 : 1;
    [ObservableProperty]
    public partial bool IsAutoStartEnable { get; set; } = SystemProfile.AutoStart;
    [ObservableProperty]
    public partial bool MinimizeWhenOpen { get; set; } = SystemProfile.MinimizeWhenOpen;
    [ObservableProperty]
    public partial string AppCacheDirectory { get; set; } = AppPathHelper.AppCache;
    [ObservableProperty]
    public partial string AppCacheSize { get; set; } = FileHelper.GetDirectorySize(new(AppPathHelper.AppCache)).FileSize();
    [ObservableProperty]
    public partial string AppDataDirectory { get; set; } = AppPathHelper.AppLocalData;
    [ObservableProperty]
    public partial string AppDataSize { get; set; } = FileHelper.GetDirectorySize(new(AppPathHelper.AppLocalData)).FileSize();
    [ObservableProperty]
    public partial string AppLogDirectory { get; set; } = AppPath.LogDictionary;
    [ObservableProperty]
    public partial string AppLogSize { get; set; } = FileHelper.GetDirectorySize(new(AppPath.LogDictionary)).FileSize();
    [ObservableProperty]
    public partial string CurrentVersion { get; set; } = Assembly.GetEntryAssembly()?.GetName().Version is Version version ? version.ToString(3) : "无法获取版本信息";
    [ObservableProperty]
    public partial string IgnoreVersion { get; set; } = SystemProfile.IgnoreVersion;
