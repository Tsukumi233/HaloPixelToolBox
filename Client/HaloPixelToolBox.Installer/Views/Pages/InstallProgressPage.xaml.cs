                ...
                default:
                    if (Assembly.GetExecutingAssembly().GetManifestResourceStream("HaloPixelToolBox.Installer.Resources.Resource.Source.zip") is Stream innerStream)
                    {
                        ...
                        if (Install(innerStream))
                            FileHelper.CreateShortCut($@"{Environment.GetFolderPath(Environment.SpecialFolder.Desktop)}\花再工具箱.lnk", Path.Combine(SystemProfile.InstallPath, "HaloPixelToolBox.Client.exe"), null, "花再工具箱快捷方式", null, SystemProfile.InstallPath);
                    }
                    break;
            }
        });
